# Serverless Lerna + Npm Monorepo Starter

A Serverless monorepo starter that uses [Lerna](https://lerna.js.org).
...
- Uses Npm Workspaces to hoist packages to the root `node_modules/` directory
...
``` bash
$ git clone https://github.com/okandeldev/MonoRepo-Serverless my-project
```
...
The `packages/` and `services/` directories are Npm.
...
This is good for keeping your Lambda packages small. But it also ensures that you can have Npm hoist all your NPM packages to the project root. Without Webpack, you'll need to disable hoisting since Serverless Framework does not package the dependencies of a service correctly on its own.
...
```bash
$ npm i new-package@1.0.0
```

Note that packages should be added by specifying the version number declared in their `package.json`. Otherwise, npm tries to find the dependency in the registry.
...
[Seed](https://seed.run) supports deploying Serverless monorepo projects that use Lerna and Npm. To enable it, add the following to the `seed.yml` in your repo root:
